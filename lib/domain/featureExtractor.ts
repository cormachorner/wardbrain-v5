...
const FEATURE_PATTERNS: Record<string, string[]> = {
  ...
  sudden_onset: [
    ...
    "explosive instant headache",
    "explosive instant onset headache",
    "came on suddenly",
    "started suddenly",
  ],
  tearing_pain: [
  ...
  unilateral_headache: [
    ...
    "unilateral pain behind one eye",
    "unilateral pain behind the eye",
    "one sided pounding headache",
    "one-sided pounding headache",
    "one sided headache",
    "one-sided headache",
    "left sided headache",
...
const FEATURE_NEGATION_PHRASES: Record<string, string[]> = {
  ...
  thunderclap: ["no thunderclap", "not thunderclap", "without thunderclap"],
  neck_stiffness: ["no neck stiffness", "no stiff neck", "without neck stiffness", "neck is not stiff"],
  neck_pain: ["no neck pain", "without neck pain"],
  photophobia: ["no photophobia", "no light sensitivity", "without photophobia"],
  phonophobia: ["no phonophobia", "no sound sensitivity", "without phonophobia"],
  rash: ["no rash", "without rash", "no non blanching rash", "no non-blanching rash"],
  seizure: ["no seizure", "no seizures", "without seizure", "without seizures"],
  reduced_consciousness: [
    "no reduced consciousness",
    "no reduced level of consciousness",
    "not drowsy",
    "not confused",
  ],
  visual_disturbance: ["no visual symptoms", "no visual disturbance", "without visual symptoms"],
  blurred_vision: ["no blurred vision", "without blurred vision"],
  diplopia: ["no diplopia", "no double vision", "without double vision"],
  visual_loss: ["no visual loss", "no loss of vision", "without visual loss"],
  transient_visual_loss: ["no transient visual loss", "without transient visual loss"],
  focal_weakness: ["no weakness", "no focal weakness", "without weakness"],
  sensory_loss: ["no numbness", "no sensory loss", "without numbness"],
  aphasia: ["no aphasia", "no word finding difficulty", "without speech disturbance"],
  ataxia: ["no ataxia", "not ataxic", "no unsteadiness"],
  dysarthria: ["no dysarthria", "no slurred speech", "speech not slurred"],
  cranial_nerve_deficit: ["no cranial nerve deficit", "no cranial nerve signs", "no facial droop"],
  papilloedema: ["no papilloedema", "no papilledema", "without papilloedema"],
};
...
function getAgeFeatures(allText: string, structuredAge: string, sex: string): string[] {
  ...
  if (ages.some((age) => age >= 50)) {
    features.push("age_over_50");
  }
